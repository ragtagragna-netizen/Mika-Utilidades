...
app.registerExtension({
  ...
  async beforeRegisterNodeDef(nodeType, nodeData, app) {
    ...
    // Un grupo se considera "bypasseado" cuando TODOS sus miembros directos
    // están en modo off.
      ...
      for (let i = 0; i < visibleGroups.length; i++) {
        ...
        if (widget) {
          ...
          widget.callback = (value) => {
            ...
            syncApply(node, node._mikaGroupMapping || []);
            if (!boolValue && entry) {
              // OFF explícito: el grupo sale del bypass aunque lo hubiera
    ...
    function tick(node) {
      ...
      let mapping = node._mikaGroupMapping || [];
      if (!mappingMatches(node, graph, mapping)) {
        rebuild(node);
        mapping = node._mikaGroupMapping || [];
      }
